...
func (r *StatisticsRepository) GetTasks(ctx context.Context, userID *int, from *time.Time, to *time.Time) ([]domain.Task, error) {
	...
	var queryBuider strings.Builder

	queryBuider.WriteString(`
...
`)
	...
	if len(conditions) > 0 {
		queryBuider.WriteString(" WHERE " + strings.Join(conditions, " AND "))
	}
	queryBuider.WriteString(" ORDER BY id ASC")
	rows, err := r.pool.Query(ctx, queryBuider.String(), args...)
	if err != nil {
